...
以下是一个适合你项目的中英双语 `README.md` 文件模板，涵盖了项目的描述、功能特点、使用方法和示例。
...
- **Excel Export / Excel 导出**:
  Exports the processed results and statistics to an Excel file.
  将处理结果和统计数据导出到 Excel 文件。
- **Designed for Academic Use / 为学术研究设计**:
  Simplifies the analysis of large-scale literature for academic researchers and professionals.
  简化了学术研究者和专业人士的大规模文献分析过程。
...
1. Python (>=3.8)
2. Required libraries / 必需库:
   - tkinter
   - pandas
   - pillow
   - pyinstaller (optional, for creating `.exe` files / 可选，用于生成 `.exe` 文件)
...
1. Clone the repository:
   ...
   ```
   bash复制代码git clone https://github.com/your_username/Web-of-science-literature-classifier.git
   cd Web-of-science-literature-classifier
   ```
...
3. Optional: Create an executable file for easier distribution (Windows):
   可选：为 Windows 平台生成可执行文件：
   ...
   复制代码
   pyinstaller --noconsole --onefile --icon=book_icon.ico your_script.py
   ```
...
### 3. Process Files / 处理文件

- Click the **"Process Files"** button to start processing the selected `.txt` files.
  单击 **“Process Files”** 按钮开始处理所选 `.txt` 文件。
- The program will display real-time progress and log messages in the text box.
  程序会在文本框中显示实时进度和日志信息。
